...
import { youtubeUrl, youtubeMedia, validateYoutubeMetadata, youtubeAudioSection } from './youtube.js';
...
test('audio range validation rejects invalid or oversized sections before starting FFmpeg', async () => {
  const audio = { source: '', title: '', duration: 7200000, audioUrl: '' };
  for (const [start, end] of [[-1, 1000], [0, 62001], [1000, 1000], [0, NaN], [7200000, 7200001]]) {
    await assert.rejects(youtubeAudioSection(audio, start!, end!, 'must-not-run', new AbortController().signal), /Ugyldig/);
  }
});
